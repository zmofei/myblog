...
then
    ...
    scp -r ./build_$FileName.tar.gz root@zhuwenlong.com:/usr/local/zhuwenlong/www/zhuwenlong.com/myblog/receive/build_$FileName.tar.gz;
    CP="cd /usr/local/zhuwenlong/www/zhuwenlong.com/myblog/receive/; rm -rf build; tar -xvzf build_$FileName.tar.gz; cd ..; cp -r receive/build/root/* ./root/; cp -r receive/build/static/* ./static/;"
